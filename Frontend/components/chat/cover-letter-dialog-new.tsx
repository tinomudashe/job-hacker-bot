...
export function CoverLetterDialog({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const fileName = `cover-letter-${selectedStyle}-${
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-5xl max-h-[92vh] sm:max-h-[95vh] w-[95vw] h-[92vh] sm:w-[90vw] sm:h-[90vh] flex flex-col !bg-white dark:!bg-background/60 dark:backdrop-blur-xl dark:backdrop-saturate-150 !border !border-gray-200 dark:!border-white/8 shadow-2xl rounded-2xl sm:rounded-2xl overflow-hidden p-0">
        {/* Header */}
        <div className="flex-shrink-0 !bg-white dark:!bg-background/60 dark:backdrop-blur-xl dark:backdrop-saturate-150 !border-b !border-gray-200 dark:!border-white/8 p-3 sm:p-5 pr-12 sm:pr-14 relative z-10">
          {/* Mobile Layout */}
...
I am particularly drawn to ${
                        editedCompanyName || "[Company Name]"
                      } because [Reason for Interest in Company]. I am excited about the opportunity to contribute to [Specific Project/Goal] and help drive [Company Objective].
...
${personalInfo.fullName || "[Your Name]"}`}
                      rows={18}
                      data-scrollable="true"
                      className="resize-none text-sm sm:text-base px-3 sm:px-4 py-2.5 sm:py-3 leading-relaxed min-h-[400px] sm:min-h-[500px] focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all duration-200 cursor-text scrollbar-thin !bg-white dark:!bg-background/80"
                      style={{
