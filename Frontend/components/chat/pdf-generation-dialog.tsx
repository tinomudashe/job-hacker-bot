...
export function PDFGenerationDialog({
  ...
  const addSkill = () => {
    if (newSkill.trim() && !skillsArray.includes(newSkill.trim()) && skillsArray.length < 15) {
  ...
  React.useEffect(() => {
    if (skills && skillsArray.length === 0) {
      const skillsFromString = skills.split(',').map(s => s.trim()).filter(s => s.length > 0)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const fileName = `${contentType === "cover_letter" ? "cover-letter" : "resume"}-${selectedStyle}-${new Date().toISOString().split('T')[0]}.pdf`
      a.download = fileName
  ...
  const isContentValid = React.useMemo(() => {
    ...
    if (contentType === "resume") {
      ...
      const hasPersonalInfo = Object.values(personalInfo).some(val => val.trim().length > 0)
      const hasWorkExperience = workExperience.some(job => 
      ...
      return hasPersonalInfo || hasWorkExperience || hasEducation || hasSkills || hasAdditionalSections
    }
    
    return false
  }, [editedContent, contentType, personalInfo, workExperience, education, skills, additionalSections])
  ...
  const getCombinedContent = () => {
    ...
    // Work Experience
    const validJobs = workExperience.filter(job => 
      job.title.trim() || job.company.trim() || job.description.trim()
    )
    if (validJobs.length > 0) {
      ...
      validJobs.forEach(job => {
        if (job.title || job.company) {
          combined += `**${job.title}** ${job.company ? `at ${job.company}` : ""}\n`
        }
        if (job.startYear || job.endYear) {
          combined += `*${job.startYear}${job.endYear ? ` - ${job.endYear}` : " - Present"}*\n`
        }
    ...
    if (validEducation.length > 0) {
      ...
      validEducation.forEach(edu => {
        if (edu.degree || edu.school) {
          combined += `**${edu.degree}** ${edu.school ? `from ${edu.school}` : ""}\n`
        }
  ...
  const formatContentForPreview = (content: string) => {
    if (!content) return '<p class="text-gray-500 italic">Start writing your content in the editor to see it here...</p>'
    ...
    formatted = formatted.replace(/(<li.*<\/li>)/g, '<ul class="list-disc list-inside mb-3 space-y-1">$1</ul>')
            ...
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <DialogTitle className="text-lg font-bold text-gray-900 dark:text-gray-100 leading-tight">
                  {contentType === "cover_letter" ? "Cover Letter" : "Resume"} Generator
                </DialogTitle>
        ...
        <div className="flex-shrink-0 border-b border-white/40 dark:border-gray-800/60 bg-gray-50/90 dark:bg-gray-900/90 backdrop-blur-md relative z-10">
          ...
          <div className="sm:hidden px-5 py-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {contentType === "cover_letter" && expandedSections.personalInfo && (
                  <>
                ...
                {contentType === "resume" && expandedSections.personalInfo && (
                  <>
                    ...
                    <span className="font-medium text-gray-900 dark:text-gray-100">Personal Info</span>
                  </>
                )}
                {expandedSections.workExperience && (
                  <>
                    <Briefcase className="h-4 w-4 text-green-600" />
                    <span className="font-medium text-gray-900 dark:text-gray-100">Work Experience</span>
                  </>
                )}
                {expandedSections.education && (
                  <>
                    <GraduationCap className="h-4 w-4 text-purple-600" />
                    <span className="font-medium text-gray-900 dark:text-gray-100">Education</span>
                  </>
                )}
                ...
                {expandedSections.additionalSections && (
                  <>
                    ...
                    <span className="font-medium text-gray-900 dark:text-gray-100">Additional</span>
                  </>
                )}
                ...
                {!Object.values(expandedSections).some(Boolean) && (
                  <>
                    ...
                    <span className="font-medium text-gray-500">Select Section</span>
                  </>
                            )}
                          </div>
              <ChevronDown className={`h-4 w-4 text-gray-500 transition-transform ${isMobileNavOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            {/* Mobile dropdown menu */}
                ...
                >
                  ...
                  {contentType === "cover_letter" ? "Cover Letter Content" : "Free-form Content"}
                </button>
                          </div>
            )}
              </div>
          ...
          <div className="h-full flex flex-col">
              ...
              <div className="flex-1 p-3 sm:p-6 bg-gradient-to-br from-gray-50/30 via-white/20 to-gray-100/30 dark:from-gray-900/30 dark:via-gray-800/20 dark:to-gray-900/40 backdrop-blur-sm">
                <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
                  ...
                  {contentType === "resume" && expandedSections.workExperience && (
                    <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md rounded-xl p-4 sm:p-6 shadow-lg border border-white/30 dark:border-gray-700/50">
                      <h2 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3">
                      ...
                      <div className="space-y-4 sm:space-y-6">
                        {workExperience.map((job, index) => (
                          <div key={index} className="relative border border-gray-200 dark:border-gray-600/30 rounded-xl p-4 sm:p-6 bg-gray-50/50 dark:bg-gradient-to-br dark:from-gray-800/90 dark:via-gray-700/80 dark:to-gray-900/90 backdrop-blur-md shadow-lg hover:shadow-xl transition-all duration-200 group">
                        ...
                        />
                          ...
                          <Button
                          onClick={() => setWorkExperience([...workExperience, { title: "", company: "", startYear: "", endYear: "", description: "" }])}
                            variant="outline"
                        ...
                        >
                          ...
                          <span className="hidden sm:inline">Add Another Job</span>
                    ...
                    <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md rounded-xl p-4 sm:p-6 shadow-lg border border-white/30 dark:border-gray-700/50">
                      ...
                      <div className="space-y-6">
                        {education.map((edu, index) => (
                          <div key={index} className="border border-gray-200 dark:border-gray-600/30 rounded-xl p-4 sm:p-6 bg-gray-50 dark:bg-gradient-to-br dark:from-gray-800/90 dark:via-gray-700/80 dark:to-gray-900/90 backdrop-blur-md shadow-lg hover:shadow-xl transition-all duration-200 group">
                            <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center gap-3">
                        ...
                        />
                          ...
                          <Button
                          onClick={() => setEducation([...education, { degree: "", school: "", year: "", description: "" }])}
                            variant="outline"
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            disabled={!newSkill.trim() || skillsArray.length >= 15}
                          >
                            <Plus className="h-4 w-4" />
                          </Button>
                </div>
                        <p className="text-xs text-gray-500 mt-2">
                          Press Enter or click + to add skills ({skillsArray.length}/15)
                        </p>
            </div>
                ...
                style={{ 
                  ...
                  {/* Additional Sections */}
                  {contentType === "resume" && expandedSections.additionalSections && (
                    <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md rounded-xl p-4 sm:p-6 shadow-lg border border-white/30 dark:border-gray-700/50">
                  ...
                  {expandedSections.content && (
                    <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md rounded-xl p-4 sm:p-6 shadow-lg border border-white/30 dark:border-gray-700/50">
                      ...
                      <Label className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">
                        {contentType === "cover_letter" ? "Cover Letter Text" : "Additional Content"}
                      </Label>
                      <Textarea
                        value={editedContent}
                        onChange={(e) => setEditedContent(e.target.value)}
                        placeholder={contentType === "cover_letter" 
                          ? `Dear Hiring Manager,
