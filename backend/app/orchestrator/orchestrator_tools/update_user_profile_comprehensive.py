...
async def _update_user_profile_comprehensive(
    db: AsyncSession, user: User, **kwargs
) -> str:
    """The underlying implementation for comprehensively updating the user profile and resume data."""
    try:
        updated_fields = []
        ...
        if isinstance(db_resume, str):  # Error case from get_or_create_resume
            return db_resume
        ...
        return "✅ Profile updated successfully:\n" + "\n".join(updated_fields) if updated_fields else "No profile information was provided to update."
        
    except Exception as e:
        log.error(f"Error in _update_user_profile_comprehensive: {e}", exc_info=True)
        await db.rollback()
        return f"❌ An error occurred while updating your profile: {e}"

# Step 3: Manually construct the Tool object with the explicit schema.
update_user_profile_comprehensive = Tool(
    name="update_user_profile_comprehensive",
    description="Updates user profile and resume personal data for consistency. Handles one or more fields.",
    func=lambda **kwargs: _update_user_profile_comprehensive(**kwargs),
